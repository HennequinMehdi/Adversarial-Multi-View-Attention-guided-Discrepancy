...
def superconduct():
    ...
    try:
        X_v1 = pd.read_csv(path + "superconductivity.csv")
        X_v2 = pd.read_csv(path + "superconductivity_graph.csv")
...
def create_domain(data):
    ...
    l         = list(data.columns).index(split_col)
    
    for i in range(-1,len(cuts)):      
        X_D.append(data.iloc[domain(data.values, cuts, l, i).tolist(),:]) 
        interval.append(domain(data.values, cuts, l, i))
    return X_D, interval


def create_views(data, nb_views):
    """
    create views
    """
    X = data
    size_views = data.shape[1]//nb_views
    X_v = []
    ...
    for v in range(nb_views-1):
        ...
        for i in columns:
            ...
            if sum(n for n in (corr_df.iloc[:,X.columns.get_loc(i)] - 0.5).sort_values(ascending=False).head(size_views).values.flatten()) > sum_corr:
                sum_corr = sum(n for n in (corr_df.abs().iloc[:,X.columns.get_loc(i)]- 0.5).sort_values(ascending=False).head(size_views).values.flatten())
                corr =i
        remove_columns = (X.corr(method='spearman').abs().iloc[:,X.columns.get_loc(corr)]-0.5).sort_values(ascending=False).head(size_views).index
        X_v.append(X[remove_columns.tolist()].to_numpy())
        X = X.drop(remove_columns,1)
        columns = X.columns.tolist()
        
    X_v.append(X.to_numpy())
        
    return X_v


def zero_padding( X_v, size):
    
    X_v_new = []
    shape = X_v[0].shape[0]
    for i in X_v:
        size_padding = (size - i.shape[1])
        X_v_new.append(np.c_[ i , np.zeros((shape,size_padding)) ])
    return X_v_new
...
def download_uci(path):
    ...
    os.rename(path + "unique_m.csv", path + "superconductivity_graph.csv")
