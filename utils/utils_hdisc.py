...
def val_split(X_s, y_s, frac_train = 0.9):
    ...
    X_train, y_train = [X_s[i][idx_train[0]]  for i in range(n_domains)], [y_s[i][idx_train[0]]  for i in range(n_domains)]
    X_val, y_val = [X_s[i][idx_val[0]]  for i in range(n_domains)], [y_s[i][idx_val[0]]  for i in range(n_domains)]
