...
class Disc_MSDANet(nn.Module):
    ...
    def __init__(self, params):
        ...
        print(self.feature_extractor.parameters())
    ...
    def train_alpha_discrepancy(self, X_s, X_t, y_s, clip=1, lam_alpha=0.01):
        ...
        #Feature training
        y_spred, y_sdisc, y_tpred, y_tdisc = self.forward(X_s, X_t)
        disc = torch.sum(torch.stack([self.alpha[i]*torch.abs(self.loss(y_spred[i], y_sdisc[i])-self.loss(y_tpred, y_tdisc)) for i in range(len(y_spred))]))
